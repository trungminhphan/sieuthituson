...
if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST"){
	if(isset($_FILES['hanghoa_files']['name']) && $_FILES['hanghoa_files']['name']){
		// Loop $_FILES to exeicute all files
		foreach ($_FILES['hanghoa_files']['name'] as $f => $name) {   
		    if ($_FILES['hanghoa_files']['error'][$f] == 4) {
		        echo 'Failed';
		        continue; // Skip file if any error found
		    } 
		    if ($_FILES['hanghoa_files']['error'][$f] == 0) {	           
		        if ($_FILES['hanghoa_files']['size'][$f] > $max_file_size) {
